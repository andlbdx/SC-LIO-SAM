 ...
 * 检查点云是否基本上是一个平面
 * 
 * @param cloud_in 输入的点云数据
 * @param thr 平面度的阈值，用于判断点云是否为平面
 * @return 如果点云基本上是一个平面，则返回true；否则返回false
 */
bool CheckIfJustPlane(const pcl::PointCloud<PointType>::Ptr& cloud_in, const float &thr){

  if (!cloud_in || cloud_in->empty()) {
      std::cout<< "输入点云数据为空,不进行平面判断..."<<endl;
  }

  // 创建SACSegmentation对象，用于从点云中分割出平面
  ...
  std::cout <<  "plane_ratio: " << ratio << std::endl;
  //如果大于阈值，则认为点云基本上是一个平面，返回true
  if (ratio > thr)
    return true;
  return false;
}
